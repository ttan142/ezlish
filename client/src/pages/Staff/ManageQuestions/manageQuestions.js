...
const CreateQuestionForm1 = () => {
  ...
  const [updatedInfo, setUpdatedInfo] = useState({ answer: '', content: '',explain:'', options: [], part: '', question:'', upload:''});
  ...
  const handleTestChange1 = async (event) => {
    ...
    try {
      const response = await axios.get(`/api/questions/${selectedTestID}`);
  ...
  const fetchQuestions = async () => {
    ...
    try {
      const response = await axios.get(`/api/questions/${testID1}`);
      const questionData = response.data.answer.flat();
      console.log(questionData);
      console.log(response.data);
      setQuestions(questionData);
  ...
  const handleQuestionClick = (question) => {
    ...
    setUpdatedInfo({ content: question.content, answer: question.answer, options: question.options, explain: question.explain, part: question.part, question: question.question, upload: question.upload });
  };
  ...
  const updateQuestion = () => {
    ...
    axios
      .put(`/api/questions/${questionId}`, updatedInfo)
      .then((response) => {
    ...
    Close
      ...
      {showToast && (
        <div
          ...
          aria-live="assertive"
          aria-atomic="true"
          style={{ position: 'fixed', top: '14vh', right: '20px', zIndex: 1 }}
        >
      ...
      <div className=" justify-content-center align-items-center text-center h-100 mt-3 mb-3 pb-3 ">
        <button className="btn btn-outline-primary w-25" onClick={() => {setShowForm(true);
        setShowForm1(false)}}>
          Create question
        </button>
        </div>
         ...
         <div className="row">
           <div className="col-lg-12 grid-margin stretch-card">
             <div className="card" style={{ borderRadius: "25px" }}>
               ...
               <div className="form-group mb-4">
                        <label htmlFor="test" className="form-label">Select Test:</label>
                        <select id="test" className="form-control" value={testID1} onChange={handleTestChange1}>
                          <option value="">Select a test</option>
                          {testNames.map((testName) => (
                            <option value={testName.value} key={testName.value}>
                              {testName.label}
                     ...
                     <thead>
                       <tr>
                         ...
                         <th
                           ...
                           style={{ width: '35vw' }}
                           className={sortConfig.key === 'content' ? `sorted-${sortConfig.direction}` : ''}
                         >
                           Content
                         </th>
                         <th
                           ...
                           className={sortConfig.key === 'answer' ? `sorted-${sortConfig.direction}` : ''}
                         >
                           Answer
                         </th>
                         <th
                         ...
                         <th
                           ...
                           style={{ width: '15vw' }}
                           className={sortConfig.key === 'options' ? `sorted-${sortConfig.direction}` : ''}
                         >
                           Options
                         </th>
                       </tr>
                     ...
                     <tbody>
                       {currentQuestions.map((question, index) => (
                         <tr
                           ...
                           className={selectedQuestion && selectedQuestion._id === question._id ? 'selected' : ''}
                 ...
                 {selectedQuestion && (
                     <div>
                       ...
                       <div className="form-group">
                         ...
                         <button type="button" className="btn btn-danger" onClick={deleteQuestion}>
                           Delete
                         </button>
                       </form>
                     </div>
                 ...
                 <div className="d-flex justify-content-between align-items-center mt-3">
                   ...
                   <div className="page-range">
                     {indexOfFirstQuestion + 1}-{Math.min(indexOfLastQuestion, sortedQuestions.length)} of {sortedQuestions.length}
                   </div>
