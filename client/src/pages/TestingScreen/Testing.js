...
function Testing() {
  ...
  useEffect(() => {
    ...
    const sum = state.current.parts.reduce(
      (total, part) => total + part.numberQuestion,
      0
    );
    console.log(sum);
  }, [state]);
  ...
  const handleSubmit = useCallback(() => {
    const userChoice = Object.values(selectedAnswers).map(
      (answer) => answer[0]
    );
    ...
    axios
      .post(
        "https://ezlish-server.onrender.com/api/results/" + state.current._id,
        {
  ...
  return (
    ...
    <div className="time-bar">
      ...
      <button
        ...
        style={{ color: "#1eb2a6", borderColor: "#1eb2a6" }}
        onClick={handleExit}
      >
        Exit
      </button>
    ...
    <div className="container1">
      <div className="question-context1">
        <div className="audio-context1">
          <div className="item1">
            <audio controls>
              <source src={state.current.audio} type="audio/mpeg" />
        ...
        {currentQuestions.map((ques) => (
          <div id="question-wrapper">
            <div id={"section".concat(ques.question)}>
              {ques.upload && <img src={ques.upload} alt="" />}
              <br />
